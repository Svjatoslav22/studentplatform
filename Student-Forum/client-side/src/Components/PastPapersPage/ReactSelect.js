...
import { saceitPaperSubjects } from '../../data/saceit';
...
export default function SimpleSelect() {
  const [, setPastPaper] = useContext(PastPaperContext);
  ...
  const showTimetable = async (e) => {
    ...
    try {
      const response = await apiClient.post('/pastpapers', Data);
    ...
    <div>
      ...
      <FormControl variant="outlined" className={classes.formControl}>
        ...
        >
          ...
          {saceitPaperSubjects.map((paperSubject) => (
            <MenuItem key={paperSubject} value={paperSubject}>
              {paperSubject}
            </MenuItem>
          ))}
        </Select>
